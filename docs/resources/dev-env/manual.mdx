...
As of this writing, the latest stable release version of Python is `3.10.6`, which will be the primary supported version for this course (other versions may work, but no promises. I wouldn't go earlier than 3.9). Make sure that this edition of Python is on your machine in one of a few ways:
...
Poetry is a modern Python package and dependency manager. If you are familiar with using `pip` to install Python package dependencies, Poetry does the same thing but also manages the file(s) that convey these requirements to the Python interpreter (similar to pipenv). 
...
- When you see `pip install` in installation instructions for a package, you should use the `poetry add` command. Remove them with `pip remove`. More Poetry commands can be found in their [documentation](https://python-poetry.org/docs/basic-usage/).
- Packages are listed in `pyproject.toml` such that the specified version (whether you personally specify which versions of the package are allowed, or Poetry does it for you) can be more generic. For example, if you are installing a package with a version constraint of `>=1.0.0`, you can use `poetry add <package> >=1.0.0`. When you manage your dependencies with Poetry commands, Poetry generates a machine-readable `poetry.lock` file that contains the exact versions of all dependencies based on all of their requirements. Don't touch this file.
